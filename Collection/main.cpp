...
#ifdef TAKE_ARGUMENTS
cxxopts::Options options("Action Tracer",
                         "This program runs a given number of MPU6050 IMU's "
                         "and sends the data packets via UDP.");
#endif
...
PI_THREAD(network_watcher) {
  ...
  communicator = new Packager(PORT); // Initialize the communicator that will
...
/**
 * @brief Initialise all the devices in the network. Store them in objects in
 * main.h
 * @return 0 if success
 */
void setup() {
  ...
  for (size_t i = 0; i < _sensors; i++) {
    ...
    body_sensor[i] = new TracePoint(
        i,
        get_pi_location(
            i + 1)); // offset by 1 is to ensure we are starting at ACT_DEVICE_1
#endif
  }
}
...
/**
 * @brief Main method that schedules the main tasks before and after data
 * collection.
 *
 * @param argc
 * @param argv
...
int main(int argc, char const *argv[]) {
  ...
  options.add_options()("f,file",
                        "Define variables using a file. If a file is given, "
                        "all other given parameters will be overwritten.",
                        cxxopts::value<std::string>()->default_value(""));
  options.add_options()("h,help", "Print usage");
  options.add_options()("t,tracepoints",
                        "Number of body devices being used on the body",
