...
public class player : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        bone_head = GameObject.Find("Head");
    ...
    void GetNetData()
    {
        IPEndPoint me = new IPEndPoint(IPAddress.Parse("192.168.137.1"), port);
    ...
    void Update()
    {
        ...
        if ( Input.GetKey(KeyCode.P)){
            ...
            Debug.Log(" X - " + vals[0,1].ToString());
            Debug.Log(" Y - " + vals[0,2].ToString());
            Debug.Log(" Z - " + vals[0,3].ToString());
            Debug.Log(" W - " + vals[0,0].ToString());
        }

        bone_head.transform.rotation = new Quaternion(vals[0, 1] + x_adj, vals[0, 2] + y_adj, vals[0, 3] + z_adj , vals[0, 0] + w_adj);
